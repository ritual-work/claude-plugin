...
the task to one leaf in `references/leaf-registry.json` and runs that leaf's path.
It does not itself do discovery — discovery lives in the Ritual engine.
...
about the work and its outcome in their terms (e.g. "let's nail the positioning",
