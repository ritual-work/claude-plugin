...
Scope **Build Backend Service** through the Backend Developer lens. This is a Ritual
discovery command — it produces a **Service Build Brief** (a decision-ready
...
across every contributing lens; the above are the Backend Developer's own.
...
2. **Run the discovery, seeded with this job.** Create a Ritual exploration for
   the request above with **jtbd `build-backend-service`** under the **backend-developer** lens,
...
3. **Speak the practitioner's language.** Talk about the work and its outcome in
   Backend Developer terms — never the routing internals (jtbd ids, registries, leaves).
