---
description: Scope "Build Backend Service" as a Backend Developer — produces a Service Build Brief (a decision-ready brief, not code).
---

# Backend Developer: Define Service Contract

Scope **Build Backend Service** through the Backend Developer lens — a Ritual discovery
command that produces a **Service Build Brief** (a decision-ready brief,
...
- Read the request through the **Backend Developer** lens and this job (**build-backend-service**).
...
1. **Ritual MCP available** (`mcp__ritual__*` tools present)? → **use it.** Create
   an exploration for the request seeded with **jtbd `build-backend-service`** under the
   **backend-developer** lens, focused on the Backend Developer sections below; walk discovery →
