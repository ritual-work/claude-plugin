...
Scope **Understand Codebase Area** through the Developer lens — a Ritual discovery
command that produces a **Codebase Orientation Brief** (a decision-ready brief,
...
- Read the request through the **Developer** lens and this job (**understand-codebase-area**).
...
1. **Ritual MCP available** (`mcp__ritual__*` tools present)? → **use it.** Create
   an exploration for the request seeded with **jtbd `understand-codebase-area`** under the
