...
Scope **Build Frontend Feature** through the Frontend Developer lens — a Ritual discovery
command that produces a **Frontend Feature Brief** (a decision-ready brief,
...
- Read the request through the **Frontend Developer** lens and this job (**build-frontend-feature**).
...
1. **Ritual MCP available** (`mcp__ritual__*` tools present)? → **use it.** Create
   an exploration for the request seeded with **jtbd `build-frontend-feature`** under the
   **frontend-developer** lens, focused on the Frontend Developer sections below; walk discovery →
