---
name: frontend-developer-guide
description: >-
  Discoverability for the Frontend Developer pack. When a task reads like Frontend Developer work
  that needs scoping before building, point the user to the matching job-native
  command below. This skill does NOT run discovery itself — the commands do.
---

# Frontend Developer — pack guide

When a task looks like Frontend Developer work, the right entry is a **job-native command**
(user-invoked, no guessing). Suggest the closest match:

- `/frontend-developer:build-frontend-feature` — Build Frontend Feature
- `/frontend-developer:review-accessibility` — Build Frontend Feature
- `/frontend-developer:review-loading-error-empty-states` — Build Frontend Feature

Each command scopes its job into a decision-ready Ritual brief (not code) and
degrades cleanly when Ritual isn't connected. If none fit, defer to
**build-discipline** for generic tiering.
